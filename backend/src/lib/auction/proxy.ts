 ...
 * Proxy-bid pricing logic.
 * Ported from functions/src/v1/services/auctions/auction.proxy.ts
 */

import { MoneyCents } from "./types";

/**
 * Minimum increment schedule (tunable).
 * Deterministic and pure — no I/O, no Date.now().
 */
...
/**
 * Reprice the proxy state after a bid change.
 *
 * Pricing rule:
 * - One bidder  => price = startPrice
 * - Two+ bidders => price = min(highMax, secondMax + inc(secondMax))
 */
