 ...
 * Zod schemas and TypeScript types for auction mechanics.
 * Ported from functions/src/v1/services/auctions/auction.types.ts
 *
 * Design constraints:
 * - No Date.now() inside business logic — caller passes nowMs explicitly.
...
/** Full snapshot of an auction used by the mechanics layer. */
...
/** Bid intent supplied by the caller. maxCents == manual bid amount for non-proxy mode. */
export const PlaceBidInputSchema = z.object({
  bidderUid: z.string().min(1),
  maxCents: MoneyCentsSchema,
});
export type PlaceBidInput = z.infer<typeof PlaceBidInputSchema>;

/** Sparse patch produced by mechanics; applied by the repo layer. */
...
/** Optimistic-concurrency guard that the repo must honour before applying a patch. */
...
/** Combined output from any compute* function: preconditions + patch. */
