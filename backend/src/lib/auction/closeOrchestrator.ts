 ...
 * Close-auction orchestrator — coordinates mechanics, repos, and logging.
 * Ported from functions/src/v1/services/orchestration/auction.close.orchestrator.ts
 *
 * Firebase-era Firestore repo imports have been replaced with minimal port interfaces
 * (ListingsRepoPort, AuctionsMetaRepoPort) so callers can inject any implementation.
 */

import { orchEvent, type OrchestrationLogger } from "./orchestrationLogging";
import { orchErr, type OrchestrationError } from "./orchestrationErrors";
import { VersionTokenSchema } from "./orchestrationTypes";

import { CloseAuctionCommandSchema, CloseAuctionResultOutSchema, type CloseAuctionResultOut } from "./closeSchemas";
import { CloseAuctionOutcomeSchema } from "./closeTypes";
...
import { computeCloseAuction } from "./mechanics";
import type { AuctionMechanicsError } from "./errors";

/** Minimal listing repo contract needed by the close orchestrator. */
type ListingsRepoPort = {
  get: (listingId: string) => Promise<{
    pricing?: { reservePriceCents?: number } | null;
  } | null>;
};

/** Minimal auction-meta repo contract needed by the close orchestrator. */
type AuctionsMetaRepoPort = {
  get: (listingId: string, auctionId: string) => Promise<{
    snapshot?: { sellerUid?: string } | null;
  } | null>;
};

/** Port interface for the aggregate auction repo. */
...
/** Dependency bundle injected into closeAuction. */
export type CloseAuctionDeps = {
  auctionsRepo: AuctionsAggregateRepoPort;
  listingsRepo: ListingsRepoPort;
  auctionsMetaRepo: AuctionsMetaRepoPort;
...
/**
 * Orchestrate closing an auction:
 * 1. Validate command input.
 * 2. Load auction core + listing + auction meta from repos.
 * 3. Run computeCloseAuction mechanics (pure).
 * 4. Apply mechanics patch via repo (optimistic concurrency).
 * 5. Determine outcome (NO_BIDS / RESERVE_NOT_MET / PAYMENT_REQUIRED).
 * 6. Return typed CloseAuctionResultOut.
 */
