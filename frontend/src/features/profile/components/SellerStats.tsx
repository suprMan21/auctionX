  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="space-y-4">
        <div>
          <label className="text-sm text-gray-600">Current Tier</label>
          <div className="mt-1">
        ...
        <div>
          <label className="text-sm text-gray-600">Lifetime Sales</label>
          <p className="text-2xl font-bold">${lifetimeSales} CAD</p>
        </div>

        <div>
          <label className="text-sm text-gray-600">Last 12 Months</label>
          <p className="text-xl font-semibold">${trailing12moSales} CAD</p>
        </div>

        {profile.seller_tier !== 'TIER_3' && (
          <div>
            <label className="text-sm text-gray-600">Progress to Next Tier</label>
            <div className="mt-2">
        ...
        <div>
          ...
          {profile.seller_verification_status === 'REJECTED' && profile.seller_verification_rejection_reason && (
            <p className="text-xs text-red-600 mt-1">{profile.seller_verification_rejection_reason}</p>
          )}
        </div>

        {profile.age_verified && (
          <div>
            <span className="text-green-600 text-sm">✓ Age verified</span>
          </div>
        )}
      </div>
    </div>
