...
async function main() {
  ...
  const startAt = Timestamp.fromMillis(nowMs - 60_000); // started 1 min ago
  const endAt = Timestamp.fromMillis(nowMs + 60 * 60_000); // ends in 60 min
  ...
  const auction: Auction = parseOrThrow(
    AuctionSchema,
    {
      ...
      // schema defaults exist, but we set explicitly for clarity
      iteration: 0,
      status: "RUNNING",
